...
class EmbeddingClient:
    ...
    def _get_local_model(self):
        if self._local_model is None:
            raise RuntimeError(
                "当前配置未启用在线嵌入 API，且已禁用本地嵌入模型加载。"
                "请配置 EMBEDDING_API_BASE / EMBEDDING_API_KEY / EMBEDDING_MODEL。"
            )
    ...
    def _get_api_client(self):
        if self._api_client is None:
            ...
            api_key = config.EMBEDDING_API_KEY.strip() or config.LLM_API_KEY.strip() or "not-needed"
