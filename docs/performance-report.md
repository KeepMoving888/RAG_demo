...
针对 200 并发压测暴露的 P95=9455ms 瓶颈，完成以下三项核心修复：

| # | 优化项 | 实现位置 | 预期效果 |
|---|--------|----------|---------|
| 1 | **JWT 用户会话缓存** | `backend/app/core/security.py` | 鉴权 DB 查询消除，~2000ms → < 5ms（Redis 优先 + 进程内 LRU 降级，TTL=10min） |
| 2 | **`/health` 轻量化** | `backend/app/main.py` | 健康检查不再查 DB，~2077ms → < 2ms；新增 `/health/db` 供 K8s readiness |
| 3 | **DB 连接池调优** | `backend/app/config.py`、`.env.example` | `pool_size` 10→20，`max_overflow` 20→40，`pool_recycle` 3600→1800 |

### 优化前后预期对比

| 指标 | 优化前（实测） | 优化后（预期） | 说明 |
|------|---------------|---------------|------|
| 单次鉴权延迟 | ~2000 ms | < 5 ms | Redis 缓存命中 |
| `/health` 延迟 | ~2077 ms | < 2 ms | 移除 DB 检查 |
| P95 延迟（200 并发） | 9455 ms | < 200 ms | 瓶颈消除后由检索链路主导（~60ms） |
| QPS（200 并发） | 27.33 | 150+ | 鉴权不再阻塞 Worker |
| 缓存命中率 | ~95% | > 95% | 检索缓存 + 用户会话缓存双轨 |

> 实际验证需启动 GPU 后端服务后重跑 `run_bench_200.bat`（4 Worker + Redis + GPU + FP16）。优化代码已全部就绪，详见 `backend/app/core/security.py` 与 `backend/app/main.py`。
